...
describe TimeTracker::Models::Config do
  ...
  describe '.find' do
    it "returns the first document in the config collection, if one exists, as a Config object" do
      @coll.save("foo" => 1)
      config = TimeTracker::Models::Config.find
  ...
  describe '.new' do
    it "stores the given document" do
      TimeTracker::Models::Config.new(:doc).doc.must == :doc
    end
  end
  describe '#save' do
    it "creates the document if it doesn't exist, setting the id to whatever the id is" do
      config = TimeTracker::Models::Config.new("foo" => 1)
      config.save
    ...
    it "updates the document if it already exists" do
      @coll.save("foo" => 1)
      doc = @coll.find_one()
      config = TimeTracker::Models::Config.new(doc)
      config["foo"] = "bar"
      config.save
      doc = @coll.find_one()
      doc["foo"].must == "bar"
