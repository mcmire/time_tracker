...
describe TimeTracker::Models::World do
  let(:world_class) { Class.new(described_class) }
  ...
  describe '.reload' do
    it "re-runs the query to fetch the config data every time" do
      mock(world_class).find { :world }.twice
  ...
  describe '.new' do
    it "stores the given document" do
      world_class.new(:doc).doc.must == :doc
    end
  end
