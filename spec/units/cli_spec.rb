...
shared_examples_for "lastfew subcommand" do
  it "prints a list of the last 4 time periods plus the currently running task, ordered by last active" do
    ...
    period1 = FactoryGirl.create(:time_period,
      :task => task1,
      :started_at => Time.zone.local(2010, 1, 1, 0, 0),
      :ended_at   => Time.zone.local(2010, 1, 2, 5, 5)
    )
    period3 = FactoryGirl.create(:time_period,
      :task => task1,
    ...
    period2 = FactoryGirl.create(:time_period,
      :task => task2,
...
describe TimeTracker::Cli do
  ...
  describe '#add' do
    context "with 'project' argument" do
      ...
      it "creates a new project" do
        @cli.run_command!("add", "project", "some project")
        Models::Project.count.must == 1
        Models::Project.first.name.must == "some project"
      end
      ...
      it "bails if the project already exists" do
        FactoryGirl.create(:project, :name => "some project")
  ...
  describe '#switch' do
    context "integration with Pivotal Tracker" do
      ...
      it "also pulls the latest tasks in the current project, if one is set, from Pivotal Tracker" do
        curr_proj = FactoryGirl.create(:project)
    ...
    it "finds the given project and sets the current one to that" do
      ...
      @cli.run_command!("switch", "some project")
      Models::Project.count.must == 1
      TimeTracker.world["current_project_id"].to_s.must == project.id.to_s
    ...
    it "creates the given project if it doesn't already exist (given user accepts prompt)" do
      ...
      Models::Project.count.must == 1
      Models::Project.first.name.must == "some project"
      TimeTracker.world["current_project_id"].must_not be_nil
    ...
    it "auto-pauses any task that's running in the current project before switching to the given one" do
      ...
      task = FactoryGirl.create(:task, :project => project, :name => "some task", :created_at => time1, :state => "running")
    ...
    it "auto-resumes a task that had been paused in this project prior to switching to another one" do
      project1 = FactoryGirl.create(:project, :name => "some project")
      task = FactoryGirl.create(:task, :project => project1, :name => "some task", :state => "paused")
      project2 = FactoryGirl.create(:project, :name => "another project")
      TimeTracker.world.update("current_project_id", project2.id.to_s)
      stdin.sneak("y\n")
      @cli.run_command!("switch", "some project")
      task.reload
  ...
  describe '#start' do
    ...
    it "bails if no project has been set yet" do
      TimeTracker.world.update("current_project_id", nil)
    ...
    context "when a task exists with the same name under the current project" do
      it "starts the clock for the task if it hasn't been started yet" do
        task = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "unstarted")
        Timecop.freeze(@time) do
          @cli.run_command!("start", "some task")
        end
        Models::Task.count.must == 1
      ...
      it "bails if the task is running" do
        FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "running")
        expect { @cli.run_command!("start", "some task") }.to raise_error("Aren't you already working on that task?")
      end
      it "bails if the task is paused" do
        FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "paused")
        expect { @cli.run_command!("start", "some task") }.to raise_error("Aren't you still working on that task?")
      end
      it "doesn't bail, but creates a new task, if the task is finished" do
        task = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "finished")
        Timecop.freeze(@time) do
          stdin.sneak("y\n")
          @cli.run_command!("start", "some task")
        end
        Models::Task.count.must == 2
        task2 = Models::Task.last(:order => :number)
  ...
  describe '#finish' do
    ...
    context "with no argument" do
      ...
      context "integration with Pivotal Tracker" do
        before do
          FactoryGirl.create(:task, :project => @project, :state => "running")
          @service = Object.new
          stub(TimeTracker).external_service { @service }
          stub(@task = Object.new).id { 5 }
          stub(@service).add_task! { @task }
      ...
      it "marks the currently running task in the current project as finished, creating a time period" do
        ...
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :created_at => started_at, :state => "running")
        task2 = FactoryGirl.create(:task, :project => @project, :name => "another task", :state => "finished")
      ...
      it "bails if all tasks under this project have been finished" do
        FactoryGirl.create(:task, :project => @project, :state => "finished")
      ...
      it "bails if none of the tasks under this project have been started yet" do
        FactoryGirl.create(:task, :project => @project, :state => "unstarted")
        expect { @cli.finish }.to raise_error("It doesn't look like you're working on anything at the moment.")
      end
      it "auto-resumes the last task under this project that was paused" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "paused", :updated_at => Time.local(2010, 1, 1, 1, 0, 0))
        task2 = FactoryGirl.create(:task, :project => @project, :name => "another task", :state => "paused", :updated_at => Time.local(2010, 1, 1, 2, 0, 0))
        task3 = FactoryGirl.create(:task, :project => @project, :name => "yet another task", :created_at => Time.local(2010, 1, 1, 3, 0, 0), :state => "running")
    ...
    context "given a string" do
      ...
      it "marks the task matching the given name as finished" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
      ...
      it "marks the last running task as finished if there are two tasks by the given name" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :updated_at => Time.local(2009, 12, 1), :state => "finished")
        task2 = FactoryGirl.create(:task, :project => @project, :name => "some task", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
        finished_at = Time.local(2010, 1, 1, 3, 29)
        Timecop.freeze(finished_at) do
          @cli.run_command!("finish", "some task")
        end
      ...
      it "bails if a task can't be found by the given name" do
        FactoryGirl.create(:task, :project => @project, :name => "another task")
      ...
      it "bails if the given task has already been finished" do
        FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "finished")
      ...
      it "doesn't bail if the given task is paused, but marks it as finished" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "paused")
        @cli.run_command!("finish", "some task")
        task1.reload
        task1.must be_finished
      ...
      it "bails if the given task is merely created" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "unstarted")
        expect { @cli.run_command!("finish", "some task") }.to raise_error("You can't finish a task you haven't started yet!")
      end
      it "auto-resumes the last task under this project that was paused" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "paused")
        task2 = FactoryGirl.create(:task, :project => @project, :name => "another task", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
        finished_at = Time.local(2010, 1, 1, 3, 29, 0)
        Timecop.freeze(finished_at) do
    ...
    context "given a number" do
      ...
      context "integration with Pivotal Tracker" do
        before do
          FactoryGirl.create(:task, :project => @project, :number => "1", :state => "running")
          @service = Object.new
          stub(TimeTracker).external_service { @service }
          stub(@task = Object.new).id { 5 }
          stub(@service).add_task! { @task }
      ...
      it "marks the task matching the given number as finished" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :number => "1", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
        finished_at = Time.local(2010, 1, 1, 3, 29)
        Timecop.freeze(finished_at) do
          @cli.run_command!("finish", "1")
        end
      ...
      it "bails if the given task has already been finished" do
        FactoryGirl.create(:task, :project => @project, :name => "some task", :number => "1", :state => "finished")
        expect { @cli.run_command!("finish", "1") }.to raise_error("It looks like you've already finished this task.")
      end
      it "doesn't bail if the given task is paused, but marks it as finished" do
        task1 = FactoryGirl.create(:task, :project => @project, :number => "1", :state => "paused")
        @cli.run_command!("finish", "1")
        task1.reload
        task1.must be_finished
        task1.time_periods.must be_empty
      ...
      it "auto-resumes the last task under this project that was paused" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :number => "1", :state => "paused")
        task2 = FactoryGirl.create(:task, :project => @project, :name => "another task", :number => "2", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
        finished_at = Time.local(2010, 1, 1, 3, 29, 0)
        Timecop.freeze(finished_at) do
  ...
  describe '#resume' do
    ...
    context "given a string" do
      ...
      #  task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "finished")
      #  @cli.run_command!("resume", "some task")
      #  task1.reload
      #  task1.must be_running
      #  stdout.must == %{Resumed clock for "some task".\n}
      ...
      #  task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "finished", :created_at => Time.zone.local(2010, 1, 1))
      #  task2 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "finished", :created_at => Time.zone.local(2010, 2, 1))
      #  @cli.run_command!("resume", "some task")
      #  task1.reload
      #  task1.must be_finished
      #  task2.reload
      ...
      it "bails if the given task can't be found in this project" do
        FactoryGirl.create(:task, :project => @project, :name => "some task")
      ...
      it "bails if a paused task can be found by that name but it's in other projects" do
        project1 = FactoryGirl.create(:project, :name => "some project")
        FactoryGirl.create(:task, :project => project1, :name => "some task", :state => "paused")
        project2 = FactoryGirl.create(:project, :name => "another project")
      ...
      it "ignores created tasks when listing other projects our paused task might be in" do
        project1 = FactoryGirl.create(:project, :name => "some project")
        FactoryGirl.create(:task, :project => project1, :name => "some task", :state => "unstarted")
        project2 = FactoryGirl.create(:project, :name => "another project")
        FactoryGirl.create(:task, :project => project2, :name => "some task", :state => "paused")
        project3 = FactoryGirl.create(:project, :name => "a different project")
        TimeTracker.world.update("current_project_id", project3.id.to_s)
      ...
      #  project1 = FactoryGirl.create(:project, :name => "some project")
      #  FactoryGirl.create(:task, :project => project1, :name => "some task", :state => "finished")
      #  project2 = FactoryGirl.create(:project, :name => "another project")
      #  FactoryGirl.create(:task, :project => project2, :name => "some task", :state => "finished")
      #  project3 = FactoryGirl.create(:project, :name => "a different project")
      #  TimeTracker.world.update("current_project_id", project3.id.to_s)
      #  expect { @cli.run_command!("resume", "some task") }.to raise_error(%{That task doesn't exist here. Perhaps you meant to switch to "some project" or "another project"?})
      #end
      ...
      it "bails if the given task is already running" do
        task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :state => "running")
    ...
    context "given a number" do
      ...
      context "integration with Pivotal Tracker" do
        before do
          FactoryGirl.create(:task, :project => @project, :number => "1", :state => "paused")
          @service = Object.new
          stub(TimeTracker).external_service { @service }
          stub(@service).check_task_exists!
          stub(@service).push_task!
      ...
      # TODO: revisit
      #it "resumes the given finished task" do
      #  task1 = FactoryGirl.create(:task, :project => @project, :name => "some task", :number => "1", :state => "finished")
      #  @cli.run_command!("resume", "1")
      #  task1.reload
      #  task1.must be_running
      #  stdout.must == %{Resumed clock for "some task".\n}
      ...
      it "bails if the given task is already running" do
        task1 = FactoryGirl.create(:task, :project => @project, :number => "1", :state => "running")
        expect { @cli.run_command!("resume", "1") }.to raise_error("Aren't you working on that task already?")
      end
      it "bails if the given task hasn't been started yet" do
        task1 = FactoryGirl.create(:task, :project => @project, :number => "1", :state => "unstarted")
      ...
      it "auto-switches to another project if given paused task is present there" do
        project1 = FactoryGirl.create(:project, :name => "some project")
        task = FactoryGirl.create(:task, :project => project1, :name => "some task", :number => "1", :state => "paused")
        project2 = FactoryGirl.create(:project, :name => "another project")
        TimeTracker.world.update("current_project_id", project2.id.to_s)
        @cli.run_command!("resume", "1")
        TimeTracker.world["current_project_id"].to_s.must_not == project2.id.to_s
        TimeTracker.world["current_project_id"].to_s.must == project1.id.to_s
      ...
      #  project1 = FactoryGirl.create(:project, :name => "some project")
      #  task = FactoryGirl.create(:task, :project => project1, :name => "some task", :number => "1", :state => "finished")
      #  project2 = FactoryGirl.create(:project, :name => "another project")
      #  TimeTracker.world.update("current_project_id", project2.id.to_s)
      #  @cli.run_command!("resume", "1")
      #  TimeTracker.world["current_project_id"].must == project1.id.to_s
      #  stdout.must == %{(Switching to project "some project".)\nResumed clock for "some task".\n}
      #end
      it "auto-pauses any task that's already running before auto-switching to another project where paused task is present" do
        project1 = FactoryGirl.create(:project, :name => "some project")
        task1 = FactoryGirl.create(:task, :project => project1, :name => "some task", :number => "1", :state => "paused")
        project2 = FactoryGirl.create(:project, :name => "another project")
        task2 = FactoryGirl.create(:task, :project => project2, :name => "another task", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
        TimeTracker.world.update("current_project_id", project2.id.to_s)
        finished_at = Time.local(2010, 1, 1, 3, 29, 0)
        Timecop.freeze(finished_at) do
          @cli.run_command!("resume", "1")
      ...
      #  project1 = FactoryGirl.create(:project, :name => "some project")
      #  task1 = FactoryGirl.create(:task, :project => project1, :name => "some task", :number => "1", :state => "finished")
      #  project2 = FactoryGirl.create(:project, :name => "another project")
      #  task2 = FactoryGirl.create(:task, :project => project2, :name => "another task", :created_at => Time.local(2010, 1, 1, 0, 0, 0), :state => "running")
      #  TimeTracker.world.update("current_project_id", project2.id.to_s)
      #  finished_at = Time.local(2010, 1, 1, 3, 29, 0)
  ...
  describe '#upvote' do
    ...
    context "given a string" do
      it "bails if no project has been set yet" do
        FactoryGirl.create(:task, :project => @project, :name => "some task", :num_votes => 3)
        TimeTracker.world.update("current_project_id", nil)
      ...
      it "increments the number of votes for the task on each call" do
        task = FactoryGirl.create(:task, :project => @project, :name => "some task", :num_votes => 3)
        @cli.run_command!("upvote", "some task")
        task.reload
    ...
    context "given a number" do
      it "bails if no project has been set yet" do
        FactoryGirl.create(:task, :project => @project, :number => 1, :num_votes => 3)
        TimeTracker.world.update("current_project_id", nil)
      ...
      it "increments the number of votes for the task on each call" do
        task = FactoryGirl.create(:task, :project => @project, :number => 1, :num_votes => 3)
        @cli.run_command!("upvote", "1")
        task.reload
        task.num_votes.must == 4
        stdout.lines.last.must == %{This task now has 4 votes.}
      ...
      it "bails if the task is finished" do
        FactoryGirl.create(:task, :project => @project, :number => 1, :state => "finished")
  ...
  describe '#list' do
    ...
    context "all" do
      it "prints a list of all tasks, ordered by last updated time" do
        ...
        period4 = FactoryGirl.create(:time_period,
          :task => task2,
        ...
        period5 = FactoryGirl.create(:time_period,
          :task => task3,
          :started_at => Time.zone.local(2010, 1, 3, 15, 15),
        ...
        period6 = FactoryGirl.create(:time_period,
          :task => task4,
  ...
  describe '#search' do
    ...
    it "compares the given query to the name of each task (not time period) in the system, and returns matching results" do
      ...
      project2 = FactoryGirl.create(:project, :name => "project 2222")
      task1 = FactoryGirl.create(:task,
        :number => "1",
        :project => project1,
        :name => "foo bar baz",
      ...
      FactoryGirl.create(:time_period, :task => task1)
      task2 = FactoryGirl.create(:task,
        :number => "21",
        :project => project2,
        :name => "foosball table",
      ...
      FactoryGirl.create(:time_period, :task => task2)
      FactoryGirl.create(:time_period, :task => task2)
      task3 = FactoryGirl.create(:task,
        :number => "3",
        :project => project1,
      ...
      FactoryGirl.create(:time_period, :task => task3)
    ...
    context "with external_service argument" do
      ...
      context "using pivotal_tracker service" do
        ...
        it "aborts if the API key is incorrect" do
          ...
          TimeTracker.world.reload
          TimeTracker.world["external_service"].must be_nil
          TimeTracker.world["external_service_options"].must be_nil
        end
        ...
        it "stores the api key and full name with the service" do
          ...
          TimeTracker.world.reload
          TimeTracker.world["external_service"].must == "pivotal_tracker"
          TimeTracker.world["external_service_options"].must == {"api_key" => "xxxx", "full_name" => "Joe Bloe"}
        end
      end
    end
